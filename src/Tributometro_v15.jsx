...
// ══════════════════════════════════════════════════════════
// PARCEIROS — DASHBOARD COMERCIAL
// ══════════════════════════════════════════════════════════
function Parceiros(){
  const {isMob}=useContext(Ctx);

  const PLIST=[
    {id:1,nome:"Ribeiro Contabilidade",uf:"SP",lat:-22.3,lon:-49.0,volume:892000,leads:43,stage:"Contrato",      mes:[4,6,5,8,7,9,8,10,12,11,14,16]},
    {id:2,nome:"Norte Fiscal",          uf:"AM",lat:-4.5, lon:-65.0,volume:341000,leads:18,stage:"Proposta",     mes:[1,2,2,3,3,4,3,5,4,6,5,7]},
    {id:3,nome:"Sudeste TRC Assessoria",uf:"MG",lat:-18.5,lon:-45.0,volume:678000,leads:31,stage:"Contrato",     mes:[3,4,5,6,5,7,6,8,9,8,10,12]},
    {id:4,nome:"Nordeste Logística",    uf:"BA",lat:-12.1,lon:-41.7,volume:215000,leads:12,stage:"Apresentação", mes:[0,1,1,2,2,2,3,2,3,4,3,4]},
    {id:5,nome:"Sul Contábil",          uf:"RS",lat:-30.0,lon:-53.1,volume:567000,leads:24,stage:"Contrato",     mes:[2,3,4,4,5,6,5,7,8,7,9,10]},
    {id:6,nome:"Centro-Oeste Fiscal",   uf:"GO",lat:-15.9,lon:-49.6,volume:143000,leads:9, stage:"Qualificação", mes:[0,0,1,1,1,2,1,2,2,3,2,3]},
    {id:7,nome:"Pantanal Assessoria",   uf:"MS",lat:-20.5,lon:-54.8,volume:289000,leads:15,stage:"Proposta",     mes:[1,2,2,3,2,4,3,4,5,4,6,5]},
    {id:8,nome:"Litoral Contábil",      uf:"RJ",lat:-22.5,lon:-43.2,volume:412000,leads:22,stage:"Contrato",     mes:[2,3,4,5,4,6,5,7,7,8,9,10]},
  ];

  const STAGE_COR={
    "Prospecção":C.text3,"Qualificação":C.blue,"Apresentação":C.amber,"Proposta":C.brand,"Contrato":C.green
  };
  const FUNIL=[
    {label:"Prospecção",  n:87},{label:"Qualificação",n:54},{label:"Apresentação",n:32},
    {label:"Proposta",    n:18},{label:"Contrato",    n:9},
  ];
  const MESES=["Jan","Fev","Mar","Abr","Mai","Jun","Jul","Ago","Set","Out","Nov","Dez"];
  const mesMax=Math.max(...PLIST.flatMap(p=>p.mes));
  const mesSum=MESES.map((_,i)=>PLIST.reduce((s,p)=>s+p.mes[i],0));
  const mesSumMax=Math.max(...mesSum);

  const BRL_=v=>"R$\u00a0"+(v/1000).toFixed(0)+"k";
  const volMax=Math.max(...PLIST.map(p=>p.volume));
  const sorted=[...PLIST].sort((a,b)=>b.volume-a.volume);

  // SVG map: lon/lat → pixel (viewBox 0 0 220 240)
  const toXY=(lon,lat)=>({
    x:((lon+73)/38)*200+10,
    y:((5-lat)/39)*220+10,
  });

  // Brazil outline (simplified polygon)
  const BR="M10,27 L26,10 L52,10 L78,10 L105,16 L131,16 L142,27 L157,27 L184,50 L210,61"+
    " L210,95 L194,123 L184,151 L173,162 L168,162 L163,174 L142,196 L121,224 L115,230"+
    " L94,230 L89,230 L89,196 L94,179 L89,162 L78,140 L89,117 L89,95 L78,83 L52,83"+
    " L36,106 L26,100 L10,83 Z";

  // Approximate state label positions
  const UFS=[
    {uf:"RR",x:68,y:22},{uf:"AP",x:118,y:27},{uf:"AM",x:52,y:68},{uf:"PA",x:116,y:62},
    {uf:"MA",x:155,y:66},{uf:"PI",x:168,y:80},{uf:"CE",x:184,y:63},{uf:"RN",x:200,y:68},
    {uf:"PB",x:200,y:76},{uf:"PE",x:195,y:83},{uf:"AL",x:202,y:90},{uf:"SE",x:199,y:96},
    {uf:"BA",x:172,y:107},{uf:"AC",x:22,y:93},{uf:"RO",x:62,y:103},{uf:"MT",x:96,y:108},
    {uf:"TO",x:138,y:95},{uf:"GO",x:131,y:128},{uf:"DF",x:148,y:130},{uf:"MG",x:157,y:142},
    {uf:"ES",x:180,y:151},{uf:"MS",x:103,y:153},{uf:"SP",x:134,y:162},{uf:"RJ",x:167,y:161},
    {uf:"PR",x:120,y:177},{uf:"SC",x:128,y:190},{uf:"RS",x:112,y:207},
  ];
  ...
  const Card=({title,sub,children,noPad})=>(
    <div style={{background:C.bg1,border:"1px solid "+C.border,display:"flex",flexDirection:"column",height:CARD_H,overflow:"hidden"}}>
      <div style={{padding:"10px 14px",borderBottom:"1px solid "+C.border,flexShrink:0}}>
        <div style={{fontFamily:F.sans,fontSize:11,fontWeight:700,color:C.text,lineHeight:1.3}}>{title}</div>
        {sub&&<div style={{fontFamily:F.sans,fontSize:8.5,color:C.text3,marginTop:2}}>{sub}</div>}
      </div>
      <div style={{flex:1,overflow:"hidden",...(noPad?{}:{padding:"10px 14px"})}}>
        {children}
      </div>
    </div>
  );
  ...
  return(
    <div style={{padding:isMob?"0":"0",background:C.bg}}>
      {/* Header */}
      <div style={{background:C.bg1,borderBottom:"1px solid "+C.border,padding:"10px 16px",display:"flex",alignItems:"center",gap:10}}>
        <span style={{fontFamily:F.sans,fontSize:13,fontWeight:700,color:C.text}}>Rede de Parceiros</span>
        <Bdg color={C.brand}>COMERCIAL</Bdg>
        <span style={{fontFamily:F.sans,fontSize:9,color:C.text3}}>Indicações · Volume · Pipeline · Distribuição</span>
      </div>

      {/* Grid de cards — todos com mesmo tamanho */}
      <div style={{display:"grid",gridTemplateColumns:isMob?"1fr":"repeat(3,1fr)",gap:1,background:C.border}}>

        {/* ─── CARD 1: MAPA DO BRASIL ─── */}
        <Card title="Distribuição Geográfica" sub={""+PLIST.length+" parceiros ativos no Brasil"} noPad>
          <svg viewBox="0 0 220 240" width="100%" height="100%"
               style={{display:"block"}}
               preserveAspectRatio="xMidYMid meet">
            {/* Fundo */}
            <rect width="220" height="240" fill={C.bg2}/>
            {/* Contorno do Brasil */}
            <path d={BR} fill="#e8f0fe" stroke={C.border} strokeWidth="0.8"/>
            {/* Labels de estados */}
            {UFS.map(s=>(
              <text key={s.uf} x={s.x} y={s.y} textAnchor="middle"
                    fontSize="5.5" fill={C.text3} fontFamily="sans-serif"
            ...
            {PLIST.map(p=>{
              ...
              const r=3+Math.sqrt(p.volume/volMax)*5;
              return(
                <g key={p.id}>
                  <circle cx={x} cy={y} r={r}
                          fill={STAGE_COR[p.stage]||C.brand}
                          stroke="#fff" strokeWidth="0.8"
                          opacity="0.88"/>
                  <text x={x} y={y-r-1.5} textAnchor="middle"
                        fontSize="5" fill={C.text} fontFamily="sans-serif"
            ...
            {/* Legenda */}
            {[["Contrato",C.green],["Proposta",C.brand],["Apresentação",C.amber],["Qualificação",C.blue]].map(([lbl,cor],i)=>(
              <g key={lbl} transform={`translate(6,${188+i*12})`}>
                <circle cx="4" cy="4" r="3.5" fill={cor} opacity="0.85"/>
                <text x="10" y="7.5" fontSize="6" fill={C.text2} fontFamily="sans-serif">{lbl}</text>
              </g>
            ))}
          </svg>
        </Card>

        {/* ─── CARD 2: VOLUME POR PARCEIRO ─── */}
        <Card title="Volume por Parceiro" sub="Faturamento mensal estimado dos clientes indicados">
          <div style={{display:"flex",flexDirection:"column",gap:7,height:"100%",overflowY:"auto"}}>
            {sorted.map((p,i)=>(
              <div key={p.id}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
                  <span style={{fontFamily:F.sans,fontSize:9,color:C.text,fontWeight:i<3?700:400,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",maxWidth:"60%"}}>
                    {i===0?"🥇 ":i===1?"🥈 ":i===2?"🥉 ":""}{p.nome}
                  </span>
                  <span style={{fontFamily:F.mono,fontSize:9,color:C.brand,fontWeight:700,flexShrink:0}}>{BRL_(p.volume)}</span>
                </div>
                <div style={{height:6,borderRadius:2,background:C.bg3,overflow:"hidden"}}>
                  <div style={{height:"100%",width:(p.volume/volMax*100)+"%",background:STAGE_COR[p.stage]||C.brand,borderRadius:2,transition:"width 0.5s"}}/>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* ─── CARD 3: RANKING DE PARCEIROS ─── */}
        <Card title="Ranking de Parceiros" sub="Classificação por leads gerados · Estágio do funil">
          <div style={{display:"flex",flexDirection:"column",gap:0,height:"100%",overflowY:"auto"}}>
            {[...PLIST].sort((a,b)=>b.leads-a.leads).map((p,i)=>(
              <div key={p.id} style={{display:"flex",alignItems:"center",gap:8,padding:"5px 0",borderBottom:"1px solid "+C.border+"66"}}>
                <span style={{fontFamily:F.mono,fontSize:9,color:C.text3,width:16,flexShrink:0,textAlign:"right"}}>{i+1}</span>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontFamily:F.sans,fontSize:9,fontWeight:600,color:C.text,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{p.nome}</div>
                  <div style={{display:"flex",alignItems:"center",gap:4,marginTop:1}}>
                    <span style={{fontFamily:F.sans,fontSize:7.5,color:C.text3}}>{p.uf}</span>
                    <span style={{width:3,height:3,borderRadius:"50%",background:STAGE_COR[p.stage]||C.text3,display:"inline-block"}}/>
                    <span style={{fontFamily:F.sans,fontSize:7.5,color:STAGE_COR[p.stage]||C.text3}}>{p.stage}</span>
                  </div>
                </div>
                <div style={{textAlign:"right",flexShrink:0}}>
                  <div style={{fontFamily:F.mono,fontSize:10,fontWeight:700,color:C.text}}>{p.leads}</div>
                  <div style={{fontFamily:F.sans,fontSize:7,color:C.text3}}>leads</div>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* ─── CARD 4: ETAPA DO FUNIL ─── */}
        <Card title="Etapa do Funil" sub="Pipeline comercial · Conversão por estágio">
          <div style={{display:"flex",flexDirection:"column",gap:8,height:"100%",justifyContent:"center"}}>
            {FUNIL.map((f,i)=>{
              const pct=(f.n/FUNIL[0].n*100).toFixed(0);
              const cor=Object.values(STAGE_COR)[i]||C.text3;
              const conv=i>0?(f.n/FUNIL[i-1].n*100).toFixed(0):null;
              return(
                <div key={f.label}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:3,alignItems:"center"}}>
                    <div style={{display:"flex",alignItems:"center",gap:6}}>
                      <span style={{width:8,height:8,borderRadius:2,background:cor,display:"inline-block",flexShrink:0}}/>
                      <span style={{fontFamily:F.sans,fontSize:9,color:C.text,fontWeight:600}}>{f.label}</span>
                    </div>
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      {conv&&<span style={{fontFamily:F.sans,fontSize:8,color:C.text3}}>{conv}% conv.</span>}
                      <span style={{fontFamily:F.mono,fontSize:10,color:cor,fontWeight:700}}>{f.n}</span>
                    </div>
                  </div>
                  <div style={{height:8,borderRadius:2,background:C.bg3,overflow:"hidden"}}>
                    <div style={{height:"100%",width:pct+"%",background:cor,borderRadius:2,transition:"width 0.6s"}}/>
                  </div>
                </div>
              );
            })}
            <div style={{marginTop:6,padding:"6px 10px",background:C.brandLt,borderRadius:2,display:"flex",justifyContent:"space-between"}}>
              <span style={{fontFamily:F.sans,fontSize:9,color:C.brand,fontWeight:600}}>Taxa de fechamento</span>
              <span style={{fontFamily:F.mono,fontSize:10,color:C.brand,fontWeight:700}}>
                {(FUNIL[4].n/FUNIL[0].n*100).toFixed(1)}%
              </span>
            </div>
          </div>
        </Card>

        {/* ─── CARD 5: INDICAÇÕES POR MÊS ─── */}
        <Card title="Indicações por Mês" sub={"Total acumulado: "+mesSum.reduce((s,n)=>s+n,0)+" indicações em 2026"}>
          <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
            <div style={{flex:1,display:"flex",alignItems:"flex-end",gap:3,paddingTop:4}}>
              {mesSum.map((n,i)=>{
                const hPct=mesSumMax>0?(n/mesSumMax*100):0;
                const isCur=i===3; // Abril = mês atual
                return(
                  <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
                    {n>0&&<span style={{fontFamily:F.mono,fontSize:6.5,color:isCur?C.brand:C.text3,fontWeight:isCur?700:400}}>{n}</span>}
                    <div style={{width:"100%",height:(hPct*1.4)+"px",background:isCur?C.brand:C.bg3,borderRadius:"2px 2px 0 0",minHeight:4,transition:"height 0.5s",position:"relative"}}>
                      {isCur&&<div style={{position:"absolute",top:-2,left:"50%",transform:"translateX(-50%)",width:4,height:4,borderRadius:"50%",background:C.brand}}/>}
                    </div>
                    <span style={{fontFamily:F.sans,fontSize:6.5,color:isCur?C.brand:C.text3,fontWeight:isCur?700:400}}>{MESES[i]}</span>
                  </div>
                );
              })}
            </div>
            <div style={{marginTop:8,display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap"}}>
              {PLIST.slice(0,3).map(p=>(
                <div key={p.id} style={{display:"flex",alignItems:"center",gap:4}}>
                  <span style={{width:6,height:6,borderRadius:1,background:STAGE_COR[p.stage]||C.text3,display:"inline-block"}}/>
                  <span style={{fontFamily:F.sans,fontSize:7.5,color:C.text3}}>{p.uf} - {p.mes[3]} ind./Abr</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* ─── CARD 6: RESUMO GERAL ─── */}
        <Card title="Resumo da Rede" sub="Visão consolidada do desempenho comercial">
          <div style={{display:"flex",flexDirection:"column",gap:10,height:"100%",justifyContent:"center"}}>
            {[
              {label:"Parceiros ativos",     val:PLIST.length,                       unit:"parceiros", cor:C.brand},
              {label:"Volume total/mês",      val:"R$\u00a0"+(PLIST.reduce((s,p)=>s+p.volume,0)/1000000).toFixed(2)+"M", unit:"faturamento indicado", cor:C.green},
              {label:"Leads no funil",        val:FUNIL.reduce((s,f)=>s+f.n,0),     unit:"oportunidades",cor:C.blue},
              {label:"Contratos fechados",    val:PLIST.filter(p=>p.stage==="Contrato").length, unit:"parceiros ativos", cor:C.green},
              {label:"Indicações em 2026",    val:mesSum.reduce((s,n)=>s+n,0),      unit:"indicações",  cor:C.amber},
              {label:"Taxa de conversão",     val:(FUNIL[4].n/FUNIL[0].n*100).toFixed(1)+"%", unit:"prospecção→contrato",cor:C.purple},
            ].map(s=>(
              <div key={s.label} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 10px",background:C.bg2,borderRadius:2}}>
                <div>
                  <div style={{fontFamily:F.sans,fontSize:8.5,color:C.text2,fontWeight:500}}>{s.label}</div>
                  <div style={{fontFamily:F.sans,fontSize:7,color:C.text3}}>{s.unit}</div>
                </div>
                <span style={{fontFamily:F.mono,fontSize:13,color:s.cor,fontWeight:700}}>{s.val}</span>
              </div>
            ))}
          </div>
        </Card>
...
// ── TABS ──
const TABS=[{id:"painel",label:"Painel"},{id:"oracle",label:"Simulador"},{id:"guia",label:"Como Usar"},{id:"parceiros",label:"Parceiros"}];
  ...
  return(
    <Ctx.Provider value={ctx}>
      <div style={{background:C.bg,minHeight:"100vh",fontFamily:F.sans,color:C.text}}>
        ...
        {/* Conteúdo */}
        <div key={tab}>{tab==="painel"?<Painel/>:tab==="guia"?<Guia/>:tab==="parceiros"?<Parceiros/>:<Oracle/>}</div>
